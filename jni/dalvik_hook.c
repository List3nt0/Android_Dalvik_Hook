...
void* dalvik_hook(struct dexstuff_t *dex, struct dalvik_hook_t *h)
{
	...
	if (h->method) {
		...
		if (h->debug_me)
			LOGI("patched %s to: 0x%x\n", h->method_name, h->native_func);

		return (void*)1;
	}
	else {
		if (h->debug_me)
			LOGI("could NOT patch %s\n", h->method_name);
	}
